...
When your `XcmsExperiment` has sample metadata (e.g., sample groups),
this is automatically carried over to `pData()` of the extracted
chromatograms. You can color each sample by a metadata column by passing
...
![Chromatograms colored by sample group
metadata.](step3-peak-correspondence_files/figure-html/color_by_metadata-1.png)

The same works for peak annotations — `peakCol` and `peakBg` also accept
column names:
...
![Chromatograms with peak fill colored by sample
group.](step3-peak-correspondence_files/figure-html/color_peaks_by_metadata-1.png)

> **How it works**
>
> When `col`, `peakCol`, or `peakBg` match a column name in
> `Biobase::pData(x)`, the value is used as a *ggplot2* aesthetic
> mapping. Otherwise it is treated as a static color string
...
![XCMS plot for XChromatograms using base R
graphics.](step3-peak-correspondence_files/figure-html/original_gplot-1.png)
...
![ggplot2 version of XChromatograms
plot.](step3-peak-correspondence_files/figure-html/xcmsvis_gplot-1.png)
...
#> [1] xcmsVis_0.99.11     patchwork_1.3.2     plotly_4.12.0      
