...
This vignette covers the **third step** in the *xcms* metabolomics workflow:
**peak correspondence** (also called peak grouping or alignment). After
detecting peaks in individual samples, these functions help you:
...
Peak correspondence groups chromatographic peaks detected across different
samples that represent the same compound. The goal is to create **features** -
groups of peaks with similar m/z and retention time that likely derive from the
same molecule.
...
The `gplotChromPeakDensity()` function helps optimize peak density
correspondence parameters by visualizing how peaks would be grouped.
...
- **Lower panel**: Peak positions as points with density curve and feature
  grouping regions (grey rectangles)
...
- **Small bandwidth (15)**: More sensitive, creates more feature groups, may
  split real features
- **Medium bandwidth (30)**: Balanced approach
- **Large bandwidth (60)**: Less sensitive, merges nearby peaks, may combine
  distinct features

### Showing Actual Correspondence Results

After running correspondence analysis, you can visualize the actual feature
grouping by setting `simulate = FALSE`:
...
When `simulate = FALSE`, the plot shows the actual feature groupings determined
by the correspondence algorithm. Vertical dashed lines indicate the median
retention time for each detected feature across samples.
:::
...
The `gplotChromatogramsOverlay()` function overlays **different EICs (rows)**
from the **same sample (column)** in one plot.
...
- **`gplot(XChromatogram)`**: Overlays the SAME m/z range across DIFFERENT
  samples
- **`gplotChromatogramsOverlay()`**: Overlays DIFFERENT m/z ranges within the
  SAME sample
:::
...
When you have multiple samples, `gplotChromatogramsOverlay()` creates a faceted
plot with one panel per sample:
...
#| fig-height: 5
#| fig-width: 8
#| fig-alt: "Chromatograms colored by sample group metadata."
...
```{r color_peaks_by_metadata}
#| fig-height: 5
#| fig-width: 8
#| fig-alt: "Chromatograms with peak fill colored by sample group."
...
```{r interactive_gplot_tooltip}
#| fig-height: 5
#| fig-width: 8
#| fig-alt: "Interactive chromatogram with sample metadata in tooltip."
...
Here's a complete workflow demonstrating how these functions work together for
...
1. **Parameter optimization**: `gplotChromPeakDensity()` helps tune
   correspondence parameters
2. **Quality control**: `gplotChromatogramsOverlay()` reveals co-eluting
   compounds within samples
3. **Sample comparison**: `gplot()` shows retention time shifts and intensity
...
→ **[Step 4: Retention Time Alignment](step4-retention-time-alignment.html)** -
...
#| fig-height: 5
#| fig-width: 5
#| fig-alt: "XCMS plot for XChromatograms using base R graphics."
plot(chr_one_eic)
```
...
#| fig-height: 5
#| fig-width: 5
#| fig-alt: "ggplot2 version of XChromatograms plot."
gplot(chr_one_eic)
```
...
#| fig-height: 5
#| fig-width: 5
#| fig-alt: "*xcms* plotChromatogramsOverlay using base R graphics."
