...
This vignette covers the **fourth step** in the *xcms* metabolomics workflow:
**retention time alignment**. After peak correspondence, these functions help
you:
...
Retention time alignment (also called retention time correction) adjusts for
systematic shifts in retention time between samples. This improves feature
detection by ensuring that the same compound elutes at consistent times across
all samples.
...
You can use only specific samples for alignment calculation while keeping all
...
LamaParama (Landmark-based Alignment Parameters) is an alignment method in
*xcms* that uses feature groups as "landmarks" to correct retention time
shifts. Unlike other methods, it explicitly models the RT relationship and
stores the alignment parameters for visualization and quality assessment.

## Landmark-Based Alignment

`LamaParama` alignment works by using a subset of high-quality features as
"landmarks" to model retention time shifts. The key is to select features that
are reliably detected across samples.
...
The x-axis shows the observed retention times in the sample, while the y-axis
shows the reference (or "Lama") retention times. The fitted line shows how
retention times should be adjusted.
...
samples. Since we filtered to features present in at least 80% of samples, these
landmarks represent robust, high-quality features suitable for modeling
...
You can use `gplotAdjustedRtime()` to visualize the overall LamaParama alignment
result:
...
→ **[Step 5: Feature Grouping](step5-feature-grouping.html)** - Group features
(isotopes, adducts)
...
#| fig-height: 5
#| fig-width: 5
#| fig-alt: "*xcms* plot(LamaParama) using base R graphics."
