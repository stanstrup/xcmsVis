...
This vignette covers the **second step** in the *xcms* metabolomics workflow:
**visualizing detected chromatographic peaks**. After running
`findChromPeaks()`, these functions help you:
...
The `gplotChromPeaks()` function creates a scatter plot showing detected peaks
as rectangles in retention time vs m/z space.
...
The `gplotChromPeakImage()` function creates a heatmap showing the number of
detected peaks per sample across retention time bins.
...
For more control, `ghighlightChromPeaks()` returns ggplot2 layers that can be
added to any chromatogram plot:
...
The `ghighlightChromPeaks()` function searches **all peaks across all
samples**. For cleaner visualization, filter to a single sample:
...
→ **[Step 3: Peak Correspondence](step3-peak-correspondence.html)** - Group
