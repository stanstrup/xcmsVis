...
#| fig-height: 4
#| fig-width: 5
#| fig-alt: "ggplot2 heatmap of peak density using viridis color scale. Retention time bins on x-axis, samples on y-axis, with color intensity indicating peak counts per bin."
# ggplot2 version
...
The `ghighlightChromPeaks()` function, like the original XCMS `highlightChromPeaks()`, takes the full `XcmsExperiment` or `XCMSnExp` object and searches **all peaks across all samples**. This can sometimes lead to unexpected behavior where peaks from multiple samples appear on a single chromatogram plot.

### Why This Might Be Confusing

When you extract a chromatogram for visualization with `chromatogram(xdata, mz = ...)`, you're typically looking at data from a specific sample. However, `ghighlightChromPeaks(xdata, ...)` will highlight **all peaks** from **all samples** in the dataset that match the specified rt/mz ranges - not just the peaks from the sample shown in the chromatogram.

### Solution: Filter to a Single Sample

For cleaner visualization that shows only peaks from the chromatogram's sample, filter the `XcmsExperiment` object first using `filterFile()`:

```{r highlight_filtered}
#| fig-height: 5
#| fig-width: 7
#| fig-alt: "Comparison showing chromatogram peak highlighting with and without sample filtering. Left panel shows all peaks from all samples, right panel shows only peaks from the specific sample."
...
The filtered approach ensures that only peaks detected in the specific sample are highlighted, which typically produces cleaner and more interpretable visualizations.
...
The polygon visualization type follows the actual peak shape from the chromatogram data:

```{r polygon_ggplot}
#| fig-height: 5
#| fig-width: 5
#| fig-alt: "ggplot2 version of polygon peak highlighting with same visual style as original, showing peaks as filled polygons."
...
This supplementary section provides side-by-side comparisons between the original XCMS plotting functions and the new xcmsVis ggplot2-based implementations. These comparisons are useful for developers and users migrating from the original XCMS plotting functions.
...
```{r original_highlightChromPeaks}
#| fig-height: 5
#| fig-width: 5
#| fig-alt: "XCMS chromatogram plot with peaks highlighted using the original highlightChromPeaks function. Shows peaks marked on the chromatogram."
...
#| fig-height: 5
#| fig-width: 5
#| fig-alt: "Original XCMS polygon peak highlighting showing detected peaks as filled polygons following the chromatogram trace within peak boundaries."
