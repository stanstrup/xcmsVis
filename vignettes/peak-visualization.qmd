...
# ghighlightChromPeaks: Annotating Chromatograms
...
#| fig-height: 5
#| fig-width: 5
#| fig-alt: "XCMS highlightChromPeaks showing base R plot of chromatogram with detected peaks highlighted as points at peak apex positions. Traditional graphics style with simple point markers. Only XCMSnExp is supported."
# Base R graphics version
...
#| fig-height: 5
#| fig-width: 5
#| fig-alt: "ggplot2 version showing chromatogram with peak annotations as composable layers. Modern styling with customizable colors and transparent fills for better visualization."
# ggplot2 version - returns layers that can be added to existing plots
p_chrom <- ggplot(chr_data, aes(x = rt, y = intensity)) +
  geom_line() +
  theme_bw() +
  labs(title = "xcmsVis ghighlightChromPeaks",
       x = "Retention Time (s)", y = "Intensity")

peak_layers <- ghighlightChromPeaks(
  xdata, chr_data, rt = rt_range, mz = mz_range,
  type = "point", border = "red"
)
...
The key difference is that `ghighlightChromPeaks()` returns ggplot2 layers that can be added to any chromatogram plot:
