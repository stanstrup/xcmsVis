...
#' @importFrom ggplot2 theme_bw labs xlim ylim facet_wrap theme element_blank
...
.gplotChromatogramsOverlay_impl <- function(object,
    ...
    if (is.null(main))
        ...
        stop("Length of 'main' must be 1 or equal to number of ",
             "columns (samples)")
    ## Get m/z values for each row if stacking is enabled
    ## stacked parameter is a proportion: y-axis is split into stacking
    ## portion and intensity portion.
    ## With stacked = 1, half the y-axis is for stacking (based on m/z),
    ## half for intensity
    ...
    ## These y-positions represent where each EIC sits on the lower
    ## portion of y-axis
    ...
    for (col_idx in seq_len(ncols)) {
        ...
        for (row_idx in seq_len(nrows)) {
            ...
            int <- intensity(chr)
            int <- transform(int)
            max_intensity_overall <- max(max_intensity_overall,
                                         max(int, na.rm = TRUE))
    ...
    ## Y-axis structure: [0, stacked] = stacking region,
    ##                   [stacked, stacked+1] = intensity region
    ## The ratio is: stacking_region : intensity_region = stacked : 1
    if (stacked > 0) {
        ## Scale intensities to fit in the intensity portion (upper part of
        ## y-axis)
        ...
        for (col_idx in seq_len(ncols)) {
            ...
            if (peakType == "point") {
                ...
                for (i in seq_len(nrow(peaks_df))) {
                    ...
                    row_idx <- pk$row
                    chr <- object[row_idx, col_idx]
                    chr_filtered <- filterRt(chr, rt = c(pk$rtmin, pk$rtmax))
...
setMethod("gplotChromatogramsOverlay", "XChromatograms",
          function(object,
                   ...
                   peakBg = NULL,
                   peakCol = NULL, peakPch = 1,
                   stacked = 0,
                   transform = identity, ...) {
              .gplotChromatogramsOverlay_impl(
                  object = object, col = col, type = type, main = main,
                  xlim = xlim, ylim = ylim, peakType = peakType,
                  peakBg = peakBg, peakCol = peakCol, peakPch = peakPch,
...
setMethod("gplotChromatogramsOverlay", "MChromatograms",
          function(object,
              ...
              .gplotChromatogramsOverlay_impl(
                  object = object, col = col, type = type, main = main,
                  xlim = xlim, ylim = ylim, peakType = "none", peakBg = NULL,
                  peakCol = NULL, peakPch = 1, stacked = stacked,
                  transform = transform, ...)
          })
