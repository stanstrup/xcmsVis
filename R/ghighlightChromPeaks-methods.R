...
.ghighlightChromPeaks_impl <- function(object,
    ...
    if (type == "rect") {
        ...
        ## For polygon, replicate the exact behavior of highlightChromPeaks
        ...
        if (nrow(pks) > 0) {
            ...
            for (j in seq_len(nrow(pks))) {
                ...
                idx <- which(chr_rt >= peak_rt_range[1] &
                             chr_rt <= peak_rt_range[2])
                if (length(idx) > 0) {
                    xs <- chr_rt[idx]
                    ys <- chr_int[idx]
