...
#' @importFrom dplyr %>% mutate filter select right_join left_join inner_join
#' @importFrom dplyr group_by group_nest rename pull all_of join_by bind_cols
...
setMethod("gplotAdjustedRtime", "XCMSnExp",
          function(object, color_by, include_columns = NULL,
                   adjustedRtime = TRUE) {
              .gplotAdjustedRtime_impl(object, {{ color_by }},
                                       include_columns, adjustedRtime)
          })
