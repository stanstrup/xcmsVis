...
.compute_peak_density <- function(pks, param) {
    ...
    densN <- max(512, 2 * 2^(ceiling(log2(diff(full_rt_range) /
                                          (bw_param / 2)))))
    dens <- density(pks[, "rt"], bw = bw_param, from = dens_from,
                    to = dens_to, n = densN)
    list(x = dens$x,
         y = dens$y,
         bw = bw_param,
...
.simulate_feature_groups <- function(pks, dens_x, dens_y, param) {
    ...
    if (length(feature_list) > 0)
        ...
        data.frame(rtmin = numeric(), rtmax = numeric(),
                   feature_id = integer())
}
...
#' @importFrom ggplot2 geom_segment theme_bw labs scale_y_continuous theme
...
.gplotChromPeakDensity_impl <-
    function(object,
        ...
        if (!any(hasChromPeaks(object)))
            stop("No chromatographic peaks present. Please",
        ...
        if (missing(param)) {
            ...
            if (hasFeatures(object)) {
                ph <- processHistory(object, type = "Peak grouping")
                if (length(ph)) {
                    ph <- ph[[length(ph)]]
        ...
        if (!is.null(features) && nrow(features) > 0) {
            ...
            warning("No feature definitions present. Either use ",
                    "'groupChromPeaks' first or set 'simulate = TRUE'")
        }
        p_lower <- p_lower +
            geom_segment(data = peaks_df,
                         aes(x = rt, xend = rt, y = 0, yend = y),
                         color = peakCol) +
            ...
            geom_line(data = dens_df,
                      aes(x = `Retention time`, y = `Peak density`)) +
            scale_y_continuous(
                breaks = ypos,
